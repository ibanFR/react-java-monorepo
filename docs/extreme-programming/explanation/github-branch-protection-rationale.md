...
[Configure GitHub Repository Protection]({% link extreme-programming/how-to/configure-github-repository-protection.md %}).
...
- A **`pre-push` hook** runs the same checks CI runs, locally, before the push leaves the machine, and aborts it if
  ...
  [Run a pre-push build gate]({% link extreme-programming/how-to/run-pre-push-build-gate.md %}).
