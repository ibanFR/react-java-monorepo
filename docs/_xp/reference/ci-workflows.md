...
[Continuous Integration]({% link _xp/explanation/continuous-integration.md %}).
...
- **`pull_request`** runs on every PR targeting `main` (no `paths` filter); the
  [path-filtering pattern]({% link _xp/reference/github-ci-path-filtering.md %}) decides at run time whether the
